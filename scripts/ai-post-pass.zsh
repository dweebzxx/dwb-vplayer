...
# Homebrew PATH — this script runs outside the /usr/bin/script session and may be
...
print "=== Duration ==="
if [[ -f "$META_FILE" ]]; then
  LAUNCH=$(grep '^LAUNCH_TIME=' "$META_FILE" | cut -d= -f2)
  COMPLETE=$(grep '^COMPLETE_TIME=' "$META_FILE" | cut -d= -f2)
...
PYEOF
    ...
    print "  LAUNCH_TIME or COMPLETE_TIME not found — was the v5.8+ launcher used?"
  fi
else
...
# Matches two formats:
#   Claude:  "1.2k tokens" or "140000 tokens"  (printed inline by claude CLI)
#   Codex:   "Token usage: total=140,062 ..."   (printed inline when --no-alt-screen is set)
# Falls back to strings(1) on the raw log when DEC private-mode sequences (\x1b[?…)
# survive ANSI stripping and prevent grep from matching the token line in the clean log.
print "=== Token Count ==="
...
if [[ -f "$META_FILE" ]]; then
  BASE_SHA="$(grep ^GIT_HEAD "$META_FILE" | cut -d= -f2)"
