...
staged_app="$staging_parent/dwb.app"
verify_root="$root_dir/.tmp/package-verify"
extracted_app="$verify_root/dwb.app"
build_info_path="$root_dir/dist/dwb-app-build-info.txt"
...
(cd "$staging_parent" && ditto -c -k --keepParent --sequesterRsrc --zlibCompressionLevel 9 "dwb.app" "$zip_path")
...
Extract \`${zip_name}\` to produce \`dwb.app\`. Because this is an ad-hoc signed, non-notarized local build, macOS Gatekeeper can reject it on first open. Use only for local verification unless a future pass produces a Developer ID signed and notarized artifact.
