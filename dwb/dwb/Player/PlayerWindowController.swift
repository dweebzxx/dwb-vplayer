...
class PlayerWindowController: NSWindowController {
    ...
    private func renameFile(at playbackSetIndex: Int, to newName: String) {
        ...
        if FileManager.default.fileExists(atPath: newURL.path) {
            DebugConsoleController.log(level: .warning, category: "rename", message: "collision: \(url.lastPathComponent) → \(newName) (target exists)")
            let alert = NSAlert()
        ...
        do {
            ...
            DebugConsoleController.log(level: .error, category: "error", message: "rename failed: \(url.lastPathComponent) → \(newName): \(error.localizedDescription)")
            let alert = NSAlert()
        ...
        DebugConsoleController.log("rename", "success: \(url.lastPathComponent) → \(newName)")
    ...
    private func scheduleReplacementStart(url: URL,
        ...
        let workItem = DispatchWorkItem { [weak self] in
            ...
            // Restore this window's audio state after media reassignment.
            // VLC may reset audio state following stop+media-swap cycles.
            let appliedAudio = self.applyWindowAudioState(reason: "reuse-player-media-replacement", force: true)
    ...
    private func applyWindowAudioState(reason: String, force: Bool = false) -> (volume: Int32, muted: Bool) {
        ...
        if force || lastAppliedAudioState?.volume != nextState.volume || lastAppliedAudioState?.muted != nextState.muted {
            player?.audio?.volume = appliedVolume
            lastAppliedAudioState = nextState
        }
        DebugConsoleController.log("audio", "apply: window=\(debugIdentity) reason=\(reason) storedVolume=\(windowVolume) appliedVolume=\(appliedVolume) muted=\(windowMuted)")
        return nextState
    }
