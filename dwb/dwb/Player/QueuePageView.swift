...
final class QueuePageView: NSView {
    ...
    func update(items: [Item], currentIndex: Int, sortMode: QueueSortMode, totalDurationText: String) {
        ...
        let selectedURLs = Set(tableView.selectedRowIndexes.compactMap { index -> URL? in
            guard index >= 0, index < self.items.count else { return nil }
            return self.items[index].url
        })
        self.items = items
        ...
        if !selectedURLs.isEmpty {
            let remapped = IndexSet(items.indices.filter { selectedURLs.contains(items[$0].url) })
            if !remapped.isEmpty {
